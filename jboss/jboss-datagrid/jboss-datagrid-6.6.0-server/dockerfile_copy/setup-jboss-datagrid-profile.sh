...
# Now that the system is running, we can run our CLI scripts. They will be executed in the order defined by 'sort -V'
	...
	do 
		...
		$JBOSS_INSTALLATION_DIR/bin/cli.sh --file=$f
	done
fi

# And we can shutdown the system using the CLI.
echo "Shutting down JBoss DataGrid."
# JBoss DataGrid CLI does not provide shutdown command, so we've written a special CLI command for it
#$JBOSS_INSTALLATION_DIR/bin/jboss-cli.sh -c ":shutdown"
$JBOSS_INSTALLATION_DIR/bin/cli.sh --file=$CLI_SCRIPTS_DIR/jdg.shutdown
